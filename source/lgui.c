...
int WinMain()
{
	...
	if(!pEnvStr){
		printerror("Env variable LGUI_HOME is Null!");
		return false;
	}
	strcpy(pString,pEnvStr);
	strcat(pString,"/pics/sys/mainback.bmp");
...
)
{
	...
	switch(message){
		case LMSG_CREATE:
		{
			...
			if(!ReadBmpToBuf(fp, SCREEN_WIDTH,SCREEN_HEIGHT, (void*)(hDCBack->pData))){
				free(pData);
				fclose(fp);
				return false;
			}
...
)
{
	...
	while(pStat){
		if(PtInRect(&(pStat->rc),x,y)){
			...
			SendMsgByServer(_lGUI_pAppStat,LMSG_PENDOWN,(void*)&point,sizeof(POINT));
				...
				if(PtInRect(&(pCRect->rect),x,y)){
					...
					if(strcmp(pWndClass->lpszClassName,"DESKICON")==0){
						...
						if(pActiveStat){//send disactive message to the application
							SendMsgByServer(pActiveStat,LMSG_IPC_DISACTIVEAPP,NULL,NULL);
						}
	...
	if(_lGUI_wndCaptureMouse.pWin){
		...
		if(pStat){
			if(PtInRect(&(pStat->rc),x,y)){
				point.x=x;
				point.y=y;
				SendMsgByServer(pStat,LMSG_PENMOVE,(void*)&point,sizeof(POINT));
				return true;
			}
		}
	}
	...
	if(_lGUI_wndCaptureMouse.pWin){
		...
		if(pStat){
			if(PtInRect(&(pStat->rc),x,y)){
				point.x = x;
				point.y = y;
				SendMsgByServer(pStat,LMSG_PENUP,(void*)&point,sizeof(POINT));
				return true;
			}
		}
	}
...
GetDeskIconInfo()
{
	...
	strcpy(pString,pEnvStr);
	strcat(pString,"/apps/appinfo.txt");
	fp = fopen(pString,"r");
	if(!fp)
		return false;
	...
	while(GetHeadTag(fp,lpszTag)){
		...
		//ID
		GetValue(fp,lpszItem,lpszValue);
		pDeskIconInfo->pTail->id = atoi(lpszValue);
		//type
		GetValue(fp,lpszItem,lpszValue);
		//name
		GetValue(fp,lpszItem,lpszValue);
		strcpy(pDeskIconInfo->pTail->lpszClassName,lpszValue);
		//icon
		GetValue(fp,lpszItem,lpszValue);
		strcpy(pDeskIconInfo->pTail->lpszIconName,pEnvStr);
		strcat(pDeskIconInfo->pTail->lpszIconName,"/pics/sys/");
		strcat(pDeskIconInfo->pTail->lpszIconName,lpszValue);
		//path
		GetValue(fp,lpszItem,lpszValue);
		strcpy(pDeskIconInfo->pTail->lpszPath,pEnvStr);
		strcat(pDeskIconInfo->pTail->lpszPath,"/apps/");
		strcat(pDeskIconInfo->pTail->lpszPath,lpszValue);
		//iTotal+1
		pDeskIconInfo->iTotal++;
		GetTailTag(fp,lpszTag);
	}
	fclose(fp);
	pDeskIconClass = pDeskIconInfo->pHead;
	while(pDeskIconClass){
		strcpy(pString,pDeskIconClass->lpszPath);
		iFileNumber = GetFileList(pString, &pFileList);
		if(strcmp(pDeskIconClass->lpszClassName,"ÎÄµµ")==0){
			for(i=0;i<iFileNumber;i++){
				pDeskIconNode = (PDeskIconNode)malloc(sizeof(DeskIconNode));
				if(!pDeskIconNode)
					return false;
				memset(pDeskIconNode,0,sizeof(DeskIconNode));
				pDeskIconNode->iTextFile = 1;

				strcpy(pDeskIconNode->lpszName,pFileList[i]->d_name);

				//strcpy(pString,pEnvStr);
				//strcat(pString,"/pics/normal_doc.bmp");
				//strcpy(pDeskIconNode->lpszIconName,pString);
				strcpy(pDeskIconNode->lpszIconName, "sys/normal_doc.bmp");


				if(!pDeskIconClass->pHead){
					pDeskIconClass->pHead = pDeskIconClass->pTail = pDeskIconNode;
				}
				else{
					pDeskIconClass->pTail->pNext = pDeskIconNode;
					pDeskIconClass->pTail = pDeskIconNode;
				}
				pDeskIconClass->iTotalIcon++;
			}//for
		}
		else{
			for(i=0;i<iFileNumber;i++){
				if(IsDeskIconInfoFile(pFileList[i]->d_name)){
					pDeskIconNode = (PDeskIconNode)malloc(sizeof(DeskIconNode));
					if(!pDeskIconNode)
						return false;
					memset(pDeskIconNode,0,sizeof(DeskIconNode));
					strcpy(pString ,pDeskIconClass->lpszPath);
					strcat(pString,"/");
					strcat(pString,pFileList[i]->d_name);
					
					fp = fopen(pString,"r");
					if(!fp)
						return false;

					GetHeadTag(fp,lpszTag);
					//comment
					GetValue(fp,lpszItem,lpszValue);
					strcpy(pDeskIconNode->lpszComment,lpszValue);
					//bin(exec)
					GetValue(fp,lpszItem,lpszValue);
					strcpy(pDeskIconNode->lpszBinName,lpszValue);
					//icon
					GetValue(fp,lpszItem,lpszValue);
					strcpy(pDeskIconNode->lpszIconName,lpszValue);
					//name
					GetValue(fp,lpszItem,lpszValue);
					strcpy(pDeskIconNode->lpszName,lpszValue);
		...
		//free file list
		FreeFileList(pFileList,iFileNumber);
		GetDeskScrollInfo(&(pDeskIconClass->iScrollMax),
			&(pDeskIconClass->iScrollPage),pDeskIconClass->iTotalIcon);

		pDeskIconClass = pDeskIconClass->pNext;
	}//while 
	return true;
}
...
)
{
	...
	strcpy(pFileSpec,(char*)(pWin->dwAddData));
	LaunchApp(pFileSpec);
